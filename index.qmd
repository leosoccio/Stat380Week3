...
1.  Reading in files
2.  Review basic dyplr functions
3.  Review basic ggplot2 functions
:::
...
Factors are categorical variables, let's look at an example:
...
2. Compute Sepal.Area as Sepal.Length times Sepal.Width
3. Find the average Sepal.Area for each species.
...
iris %>%
  ...
  group_by(Species)%>%
  summarize(Average = mean(Sepal.Area))
```

Consider a more difficult task:
1. Take a number i
